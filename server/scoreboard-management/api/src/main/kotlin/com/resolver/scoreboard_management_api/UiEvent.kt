...
import com.resolver.ProblemResult as ProblemResultCore
import com.resolver.ScoreboardRow as ScoreboardRowCore
import com.resolver.UiEvent as UiEventCore
...
fun ProblemResult.toCore(): ProblemResultCore = when (this) {
    is ICPCProblemResult -> com.resolver.ICPCProblemResult(
    ...
    is IOIProblemResult -> com.resolver.IOIProblemResult(
        score = score,
        lastSubmitTime = lastSubmitTime,
...
fun TeamId.toCore() = com.resolver.TeamId(value)
fun Collection<TeamId>.toCore() = map(TeamId::toCore)
fun OrganizationId.toCore() = com.resolver.OrganizationId(value)
fun GroupId.toCore() = com.resolver.GroupId(value)
fun ProblemId.toCore() = com.resolver.ProblemId(value)

fun Award.toCore() = when (this) {
    is Award.Custom -> com.resolver.Award.Custom(
        id = id,
        citation = citation,
        teams = teams.toCore().toSet()
    )

    is Award.GroupChampion -> com.resolver.Award.GroupChampion(
        id = id,
        citation = citation,
    ...
    is Award.Medal -> com.resolver.Award.Medal(
        id = id,
        citation = citation,
        teams = teams.toCore().toSet()
    )

    is Award.Winner -> com.resolver.Award.Winner(
        id = id,
        citation = citation,
        teams = teams.toCore().toSet()
    )
...
fun ContestResultType.toCore() = when (this) {
    ContestResultType.ICPC -> com.resolver.ContestResultType.ICPC
    ContestResultType.IOI -> com.resolver.ContestResultType.IOI
}

fun ProblemInfo.toCore() = com.resolver.ProblemInfo(
    id = id.toCore(),
...
fun TeamInfo.toCore() = com.resolver.TeamInfo(
    id = id.toCore(),
    fullName = fullName,
    displayName = displayName,
...
fun OrganizationInfo.toCore() = com.resolver.OrganizationInfo(
    id = id.toCore(),
    displayName = displayName,
    fullName = fullName
)
...
fun ContestInfo.toCore() = com.resolver.ContestInfo(
    name = name,
