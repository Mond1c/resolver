...
class ServerImplTest {
    @Test
    fun `GIVEN resolution in process WHEN client joins THEN no one of important ui events is lost`() =
        runBlocking {
            ResolverUtilComponent.provideAppBase(
                ...
                launch(SupervisorJob()) {
                    HttpClient(CIO) { install(WebSockets) }.use {
                        it.webSocket(
                            options.resolutionWsEndpoint.createWsUrlString(
                                options.host,
                                options.port
                            )
                        ) {
                            isReady1 = true
                            scoreboard1 =
                                json.decodeFromString(incoming.receive().data.decodeToString())
                            launch {
